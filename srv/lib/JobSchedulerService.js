...
/**
 * SAP BTP Job Scheduler Integration Service (Destination-based)
 * 
 * Uses standard CAP rest connection to consume BTP Job Scheduler
 * via BTP Destinations (configured as 'jobscheduler-api' in cockpit).
 * Falls back to Mock local implementation in development.
 */
...
module.exports = {
    ...
    /**
     * Creates a new job with an initial cron schedule
     */
    async createJob({ name, description, actionUrl, cronExpression, active = true }) {
        // Get UAA credentials for OAuth authentication
        let uaaCredentials = null;
        if (process.env.VCAP_SERVICES) {
            try {
                const vcap = JSON.parse(process.env.VCAP_SERVICES);
                const xsuaaService = vcap.xsuaa?.[0];
                if (xsuaaService) {
                    uaaCredentials = xsuaaService.credentials;
                }
            } catch (err) {
                console.error('[JobSchedulerService] Failed to parse XSUAA credentials:', err.message);
            }
        }
        ...
        const payload = {
            ...
            schedules: [
                {
                    cron: cronExpression || '* * * * * */60 0',
                    description: `Cron schedule for ${name}`,
                    active: true
                }
            ]
        };

        // Add OAuth authentication for job action endpoint
        if (uaaCredentials) {
            payload.actionAuthentication = {
                type: 'oauth2',
                oauth2: {
    ...
    /**
     * Configures/updates the cron schedule for a job
     */
    async updateSchedule(jobId, scheduleId, { cronExpression, active }) {
        const payload = {
            cron: cronExpression,
            active: active ?? true
        };
        ...
        if (IS_MOCK) {
            console.log(`[JobSchedulerService] [MOCK MODE] PUT /scheduler/jobs/${jobId}/schedules/${scheduleId} (cron: ${cronExpression})`);
            return { success: true };
        }
