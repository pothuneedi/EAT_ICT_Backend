...
const updatedPoLines = poLines.map((line, idx) => {
    if (idx === 0) return poLines[0]; // already updated header
    if (!line.trim()) return line;    // preserve empty trailing lines

    const cols = line.split(';');
    const headerCols = poLines[0].split(';').length;

    // If this row already has the right column count, leave it
